...
OpenRouter LLM response against a hand-written gold reference answer, for a
set of 20 static (RAG-answerable) queries grounded in the ECG + PURC
knowledge base (backend/eval/test_response_quality.json).

Requires:
  - OPENROUTER_API_KEY set in backend/.env (makes real network calls to OpenRouter)
...
load_dotenv(BACKEND_DIR / ".env")  # chatbot.py reads OPENROUTER_API_KEY at import time, so this must run first

import chatbot            # noqa: E402
import database as db      # noqa: E402

if not chatbot.OPENROUTER_API_KEY:
    sys.exit(
        "OPENROUTER_API_KEY is not set (checked backend/.env). Response-quality evaluation needs "
        "the real OpenRouter-backed pipeline, not the no-key fallback message, so it cannot proceed."
    )
