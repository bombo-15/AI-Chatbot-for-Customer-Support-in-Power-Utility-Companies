...
frozen at March 2026. This script fills ecg_outages with a hand-built set of
entries shaped exactly like what scrape_outages() would return, dated
relative to "now" so the board looks current whenever this is run.
...
it only gets replaced by a future successful scrape or another run of this
script.

Usage:
    python seed_demo_outages.py
"""
...
def main() -> None:
    ...
    print(f"Seeded {count} demo outages into ecg_outages, dated relative to {now.isoformat(timespec='seconds')}")
